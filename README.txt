...
우리조가 코딩한 Reverse 알고리즘은 한 char변수에 저장된 문자를 다른 곳에 옮겨 저장하는 과정을 3/2d + 3/2(n - d) + 3/2n = 3/2n + 3/2n = 3n번(두 문자를 swap하는 과정을 하나의 연산과정으로 본다면 n번의 연산을 거친다.) 거치므로 시간복잡도는 O(n)라 할 수 있다.
